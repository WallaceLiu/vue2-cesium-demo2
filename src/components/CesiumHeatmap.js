(function(a, b, c) {
	...
	var c = (function i() {
		...
		var c = function(a) {
			...
			var e = a.radi;
			var a = a.data;
			var f = Object.keys(a).map((e) => parseFloat(e));
			var g = f.length;
			while (g--) {
				var h = f[g];
				var i = Object.keys(a[h]).map((e) => parseFloat(e));
				var j = i.length;
				while (j--) {
					var k = i[j];
					var l = a[h][k];
	...
	var CesiumHeatmap = {
		defaults: {
			...
			spacingFactor: 3, // extra space around the borders (point radius multiplied by this number yields the spacing)
	 ...
	 */
	function CHInstance(params) {
		let c = params.viewer;
		let o = params.options;
		let data = params.data;
		if (!c) {
			throw 'viewer 参数是必须参数！';
		}
		if (!data) {
			throw 'data 参数是必须参数！';
		}
		if (!o) {
			o = {};
			console.warn('未输入热力图的配置参数，采用默认参数！');
		}
		...
		for (let i = 0; i < data.length; i++) {
			let { x, y, value } = data[i];
			if (y > bb.north) bb.north = y;
			if (y < bb.south) bb.south = y;
			if (x < bb.west) bb.west = x;
			if (x > bb.east) bb.east = x;
		...
		this._heatmap = g.create(this._options);
		this._container.children[0].setAttribute('id', this._id + '-hm');

		// 加载数据
		this.setWGS84Data(min, max, data);
	}
	...
	CHInstance.prototype.wgs84PointToHeatmapPoint = function(p) {
		return this.mercatorPointToHeatmapPoint(
			CesiumHeatmap.wgs84ToMercator(p)
		);
	};
